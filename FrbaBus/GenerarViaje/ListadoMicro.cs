...
namespace FrbaBus.GenerarViaje
{
    public partial class ListadoMicro : BaseForm
    {
        ...
        private void ListadoMicro_Load(object sender, EventArgs e)
        {
            ...
            dt = cc.executeQuery("SELECT MICRO_PATENTE AS 'PATENTE', MARCA_NOMBRE AS 'MARCA' , MICRO_CANTIDAD_KGS AS 'CANT KGS', TIPO_SERVICIO_NOMBRE AS 'TIPO SERVICIO' FROM BUGDEVELOPING.MICRO JOIN BUGDEVELOPING.MARCA ON (MICRO_CODIGO_MARCA = MARCA_CODIGO) JOIN BUGDEVELOPING.TIPO_SERVICIO ON (TIPO_SERVICIO_CODIGO = MICRO_TIPO_SERVICIO)");
